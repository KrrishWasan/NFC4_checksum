...
const handleWalletConnect = async (walletName) => {
  ...
  try {
    ...
    const nonceRes = await fetch(`http://localhost:5000/nonce/${account}`)
    const { nonce } = await nonceRes.json()

    // 3️⃣ Ask MetaMask to sign the nonce
    ...
    // 4️⃣ Send address & signature to backend for verification
    ...
    // 5️⃣ Store the token in AuthContext
